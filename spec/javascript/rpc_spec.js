...
JS.ENV.RpcSpec = JS.Test.describe("RPC", function() { with(this) {
  ...
  describe("rpcCall", function() { with(this) {
    before(function() { this.createConnectedClient() })

    // it("performs a subscription to reply channel", function(resume) { with(this) {
    //   expect(transport, "send").given(envelope({
    //     channel: "/meta/subscribe",
    //     clientId: "fakeid",
    //     subscription: "/fakeid/rpc/messages/foo",
    //     id: instanceOf("string")
    //   }));
    //   client.rpcCall("/messages/foo", {hello: "world"});
    //   client.connect(resume);
    // }})

    it("publishes rpc request to specified channel", function(resume) { with(this) {
      expect(transport, "send").given(envelope({
        channel:  "/messages/foo",
        clientId: "fakeid",
        data:     {msgId: instanceOf("string"),
                   replyChannel: "/fakeid/rpc/messages/foo",
                   payload: {hello: "world"}},
        id:       instanceOf("string")
      }))
      client.rpcCall("/messages/foo", {hello: "world"});
  ...
  describe("rpcEndpoint", function() { with(this) {
    it("performs a subscription on the specified channel")
    describe("when result is an Array", function() { with(this) {
      it("publish a successful response on replyChannel")
      it("publish an error response on replyChannel")
    }})
    describe("when result is a Promise" , function() { with(this) {
      it("publish a successful response on replyChannel")
      it("publish an error response on replyChannel")
    }})
  }})
